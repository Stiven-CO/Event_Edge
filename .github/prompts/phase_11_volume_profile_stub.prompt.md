...
mode: agent
description: >
  Fase 11 — Volume Profile stub de Event Edge: crea el módulo documentado
  sin implementación, listo para v2.
tools:
  - read_file
  - create_file
---
...
- Fase 1 completada: estructura de directorios `backend/core/` existe

## Objetivo

Crear `backend/core/volume_profile/__init__.py` como **stub documentado**.
No hay implementación en v1. El objetivo es documentar la interfaz planificada
para que v2 pueda implementarse sin romper contratos existentes.

## Archivo a crear

### `Event_Edge/backend/core/volume_profile/__init__.py`
...
```python
# El módulo es importable sin errores
from backend.core.volume_profile import __all__
assert __all__ == []

# No hay código ejecutable — solo docstring y __all__
import inspect
import backend.core.volume_profile as vp
source = inspect.getsource(vp)
assert "NotImplementedError" in source  # La interfaz documenta el error
assert "VolumeProfileCalculator" in source  # La interfaz planificada está documentada
print("Volume Profile stub OK")
```

## Restricciones

- Cero código ejecutable (excepto la asignación `__all__ = []`)
- El stub NO importa pandas, scipy ni ninguna dependencia — evitar imports innecesarios
- NO crear subarchivos en el directorio (`calculator.py`, etc.) — todo en `__init__.py`
- La docstring de la interfaz planificada usa comillas simples internas para no romper
  la docstring externa con triple comillas
